document.getElementById('applicationButton').addEventListener('click', applyInputs);
document.getElementById('drawInformationButton').addEventListener('click', () => {
    if (applyInputs()) {
        drawAmazonData()
            .then(data => {
                displayData(data);
...
function applyInputs() {
    ...
    if (!hasUppercase || !hasNumber) {
        isValid = false;
        message += 'Password must contain at least one uppercase letter and one number.<br>';
    }
...
function drawAmazonData() {
    ...
    return fetch("https://Real-Time-Amazon-Data.proxy-production.allthingsdev.co/v2/search?query=Phone&page=1&country=US&sort_by=RELEVANCE&product_condition=ALL&category_id=null&min_price=105&max_price=110&brand=null", requestOptions)
